	...
	shape.prototype.update_uniforms = function( graphicsState, model_transform, material )			// Send the current matrices to the shader
		{
				...
				var N_LIGHTS = 9, lightPositions = [], lightColors = [], attenuations = [], 
				                lightPositions_flattened = [], lightColors_flattened = [];
					
				lightPositions.push( vec4( 0, 0, 0, 1 ) );    
				lightColors.push( vec4( .5, .5, .5, 1 ) );   
				attenuations.push( .000001 );

				lightPositions.push( vec4( 5, 5, 5, 1 ) );    
...
// Customized compound shape: rocket
function rocket( points_transform ) 
{
	shape.call(this); // Inherit class shape’s array members by calling parent constructor
...
inherit(rocket, shape);
	rocket.prototype.populate = function( recipient, points_transform )
	{
	  	var m_tube = new cylindrical_strip(); 
		var m_fan = new triangle_fan_full;

	 	var stack = [];
		var object_transform = points_transform;

		stack.push(object_transform);
			object_transform = mult( object_transform, scale(0.5, 5, 0.5));
			object_transform = mult( object_transform, rotate(90, 1, 0, 0));
			m_tube.populate( recipient, 10, object_transform ); 
		object_transform = stack.pop();

		stack.push( object_transform );
			object_transform = mult( object_transform, translate(0, 3, 0)); 
			object_transform = mult( object_transform, scale(.5, .5, .5));
			object_transform = mult( object_transform, rotate(-90, 1, 0, 0));
			m_fan.populate( recipient, 10, object_transform );
		object_transform = stack.pop();
		
		stack.push( object_transform );
			object_transform = mult( object_transform, translate(0, -2.25, 0) ); 
			object_transform = mult( object_transform, scale(0.75, 1, 0.75) );
			object_transform = mult( object_transform, rotate(-90, 1, 0, 0) );
			m_fan.populate( recipient, 10, object_transform );
		object_transform = stack.pop();
	};
...
// CUSTOMIZED polygon: used for spaceship body, flat shading!!!
function polygon( points_transform )
{
	shape.call(this); // Inherit class shape’s array members by calling parent constructor
	if( !arguments.length) return;
	this.populate( this, points_transform ); 
...
inherit(polygon, shape);
	...
	polygon.prototype.populate = function( recipient, points_transform )        // The meat of the triangle starts here:
    {
        var offset = recipient.vertices.length;        var index_offset = recipient.indices.length;                // Recipient's previous size
            
        var res= -3;
        var y_res = 5;
        recipient.vertices = [  vec3(0,0 +y_res,res), vec3(2,.3 +y_res,res), vec3(2,1 +y_res,res),
        						vec3(0,3 +y_res,res), vec3(-2,1 +y_res,res), vec3(-2,.3 +y_res,res),
        						vec3(0,0 +y_res,.4 +res),

        						// tail
        						vec3(0,-.2 - .45 +y_res,1 +res), vec3(0,.2 - .5 +y_res,1 +res),
        						vec3(.2, .075-.45 +y_res,.09 +res), vec3(-.2, .075-.45 +y_res,.09 +res), vec3(0, .75-.3 +y_res,.09 +res),
        						// 0: 12
        						vec3(0,0 +y_res,res),
        						// 3: 13 ~ 16
        						vec3(0,3 +y_res,res),vec3(0,3 +y_res,res),vec3(0,3 +y_res,res),vec3(0,3 +y_res,res),
        						// 1: 17 ~ 19
        						vec3(2,.3 +y_res,res), vec3(2,.3 +y_res,res), vec3(2,.3 +y_res,res),
        						// 6: 20 ~ 22
        						vec3(0,0 +y_res,.4 +res), vec3(0,0 +y_res,.4 +res), vec3(0,0 +y_res,.4 +res),
        						// 5: 23 ~ 25
        						vec3(-2,.3 +y_res,res), vec3(-2,.3 +y_res,res), vec3(-2,.3 +y_res,res)
        						];
...
function spaceship( points_transform )
{
	shape.call(this); // Inherit class shape’s array members by calling parent constructor
	if( !arguments.length) return;
	this.populate( this, points_transform ); 
...
inherit(spaceship, shape);

	spaceship.prototype.populate = function( recipient, points_transform )
	{
	  	var m_rocket = new rocket(); 
		var m_wings = new polygon();
		var m_body = new cylindrical_strip();
		var m_head = new triangle_fan_full;
	 	var stack = [];
		var object_transform = mat4();

		var res = -3;
		var y_res = 5;
		m_wings.populate( this, object_transform );
		
		// ROCKET 1
		stack.push(object_transform);
			object_transform = mult( object_transform, translate(1.3, 1 +y_res, -.25 +res));
			object_transform = mult( object_transform, scale(.5, .5, .5));
			m_rocket.populate( this, object_transform ); 
		object_transform = stack.pop();
		...
		// BODY
		stack.push(object_transform);
			object_transform = mult( object_transform, translate(0, 1.5 +y_res, 0 +res));
			object_transform = mult( object_transform, scale(.5, 4, .4));
			object_transform = mult( object_transform, rotate(90, 1, 0, 0));
			m_body.populate( this, 10, object_transform );
		object_transform = stack.pop();		

		// HEAD
		stack.push(object_transform);
			object_transform = mult( object_transform, translate(0, 4.2 +y_res, 0 +res));
			object_transform = mult( object_transform, scale(.5, .7, .4));
			object_transform = mult( object_transform, rotate(-90, 1, 0, 0));
			m_head.populate( this, 10, object_transform );
		object_transform = stack.pop();	

			object_transform = mult( object_transform, translate(.3, -.5 +y_res, 0 +res));
			object_transform = mult( object_transform, scale(.3, .4, .2));
			object_transform = mult( object_transform, rotate(-90, 1, 0, 0));
			m_head.populate( this, 10, object_transform );

			object_transform = mult( translate(-.6, 0, 0), object_transform );
			m_head.populate( this, 10, object_transform );
	};


function spaceship_no_rocket( points_transform )
{
	shape.call(this); // Inherit class shape’s array members by calling parent constructor
	if( !arguments.length) return;
	this.populate( this, points_transform ); 
...
inherit(spaceship_no_rocket, shape);

	spaceship_no_rocket.prototype.populate = function( recipient, points_transform )
	{
	  	var m_rocket = new rocket(); 
		var m_wings = new polygon();
		var m_body = new cylindrical_strip();
...
function ring( points_transform, num_of_vertices, inner_r, outer_r )
{
	shape.call(this); // Inherit class shape’s array members by calling parent constructor
	if( !arguments.length) return;
	this.populate( this, points_transform, num_of_vertices, inner_r, outer_r ); 
	this.init_buffers(); // Then sends its arrays to the graphics card into new buffers
}
inherit(ring, shape);

	ring.prototype.populate = function( recipient, points_transform, num, inr, outr )        // The meat of the triangle starts here:
    {
        var offset = recipient.vertices.length;        var index_offset = recipient.indices.length;                // Recipient's previous size
            
        for (var i = 0; i < num; i++) {
        	var theta = 2*Math.PI/ (num-1); // angle of each piece
        	var x_cor = inr * Math.cos(i*theta);
        	var y_cor = inr * Math.sin(i*theta);
        	recipient.vertices.push( vec3(x_cor, y_cor, 0) );
        	recipient.normals.push( vec3( 0,0,1 ) ); 
        }
        for (var i = 0; i < num; i++) {
        	var theta = 2*Math.PI/ (num-1); // angle of each piece
        	var x_cor = outr * Math.cos(i*theta);
        	var y_cor = outr * Math.sin(i*theta);
        	recipient.vertices.push( vec3(x_cor, y_cor, 0) );
        	recipient.normals.push( vec3( 0,0,1 ) ); 
        }

        recipient.indices.push( 0 );
        recipient.indices.push( num - 1 );
        recipient.indices.push( num );
        ...
        for (var i = 0; i < num-1; i++) {
        	...
        	recipient.indices.push( i + 1 );
        	recipient.indices.push( i + num + 1 );
    		recipient.texture_coords.push( vec2( 0, 0 ) ); 
       		recipient.texture_coords.push( vec2( 0, 1 ) );
       		recipient.texture_coords.push( vec2( 1, 0 ) );        
       	}

        recipient.indices.push(2*num - 1);
        recipient.indices.push( num );
        recipient.indices.push(num - 1);
        recipient.texture_coords.push( vec2( 0, 0 ) ); 
        recipient.texture_coords.push( vec2( 0, 1 ) );
        ...
        for (var i = num; i < 2*num - 1; i++) {
        	recipient.indices.push( i );
        	recipient.indices.push( i + 1 );
        	recipient.indices.push( i - num );
			recipient.texture_coords.push( vec2( 0, 0 ) ); 
        	recipient.texture_coords.push( vec2( 0, 1 ) );
        	recipient.texture_coords.push( vec2( 1, 0 ) );
        }
